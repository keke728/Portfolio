...
function mouseleave() {
    ...
    d3v3
        ...
        .duration(1e3)
        .style("opacity", 1)
        .each("end", function () { d3v3.select(this).on("mouseover", mouseover) })
}
...
function i(a) {
    ...
    d
        ...
        .style("fill", function (a) { return getcolor(d3v3.rgb(a._color)) < 150 ? "#fff" : "#000" })
        .text(function (a) { return a.key }),
    c
        .attr("transform", function (a, b) { return "translate(" + b * (r.w + r.s) + ", 0)" }), 
    c.exit().remove(), 
    d3v3.select(".trail").style("visibility", "")
}
...
function k(a) {
    ...
    if (a.depth > 1) {
        var f = d[a.value % 3];
        return d3v3.rgb(a.parent._color).brighter(.2 * a.depth + f * a.depth)
    }
}
var l;
var chart = function (d3v3) {
    ...
    function c(b, c) {
        j.domain(d3v3.extent(b, function (a) { return a.date }));
        k
            .domain([0, 100]), 
            ...
            //.attr("transform", function(d) { return "translate(" + j(d.date) + "," + k(d.p) + ")"; })
            .attr("d", n)
    ...
    function refreshChart(data) {
        var e = processdata(data),
            f = d3v3.select("#skills-chart-line");
        null === f[0][0] 
        ? c(e, data) 
        : f
            .datum(e)
    ...
    var chart = {},
        ...
        i = [1, 1.01, 1.99,2, 2.01, 2.99,3, 3.01, 3.99, 4,4.01,4.99, 5, 5.01, 5.99],
        j = d3v3.scale.linear().range([0, g]),
        k = d3v3.scale.linear().range([h, 0]),
        bottomtick = d3v3
            .svg
            .axis()
            .scale(j)
            .tickValues([h,h,h,h,h])
            //.tickFormat(function(){return ["hi"]})
            .tickPadding(10)
            .tickSize(0)
            .orient("bottom"),
        lefttick = d3v3
            ...
            .orient("left"),
        n = d3v3.svg.area()
                  .x(function(a) {return j(a.date)})
                  .y0(h)
                  .y1(function(a) {return k(a.p)}),
        cpath = d3v3
            .select(".skills-chart")
            .append("svg")
    ...
    proficiencydata = d3v3
        ...
        .children(function (a) {
            return a.value instanceof Array 
                ? (a._proficiency = a.value, d3v3.entries([a.value[a.value.length - 1]]))
                : (a._proficiency = u(a.value), isNaN(a.value) ? d3v3.entries(a.value) : null)
        })
...
path.
    ...
    .attr("dx", "6").attr("dy", ".1em").attr("font-size","8px").attr("font-family","Sofia").text(function (a) { return a.key })
